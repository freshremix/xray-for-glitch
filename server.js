const url = ("https://lively-red-rabbit.cyclic.app");
const port = 3000;
...
app.get("/nezha", (req, res) => {
  let cmdStr = "/bin/bash nezha.sh lively-red-rabbit.cyclic.app 5555 dfzPfEOCA3DCAVhM4s >/dev/null 2>&1 &";
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
...
function keepalive() {
  // 1.请求主页，保持唤醒
  let app_url = "https://lively-red-rabbit.cyclic.app";
  ...
  exec("curl -m5 " + url + "/status", function (err, stdout, stderr) {
    ...
    if (!err) {
      if (stdout.indexOf("nezha-agent") != -1) {
        ...
        //哪吒未运行，命令行调起
        exec(
          "/bin/bash nezha.sh lively-red-rabbit.cyclic.app 5555 dfzPfEOCA3DCAVhM4s >/dev/null 2>&1 &",
          function (err, stdout, stderr) {
            if (err) {
...
function download_web(callback) {
  let fileName = "web.js";
  let web_url = "lively-red-rabbit.cyclic.app";
